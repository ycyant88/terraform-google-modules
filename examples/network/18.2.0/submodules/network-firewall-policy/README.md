...
| target\_vpcs | [DEPRECATED] List of target VPC IDs that the firewall policy will be attached to. Use `target_vpcs_map` instead to avoid errors with dynamically created VPCs. | `list(string)` | `[]` | no |
| target\_vpcs\_map | Map of target VPCs that the firewall policy will be attached to. Keys are static strings (e.g., 'prod-vpc'), values are the VPC IDs. | `map(string)` | `{}` | no |
