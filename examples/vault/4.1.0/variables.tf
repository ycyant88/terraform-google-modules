variable "allow_public_egress" {
  description = "Whether to create a NAT for external egress. If false, you must also specify an http_proxy to download required\nexecutables including Vault, Fluentd and Stackdriver\n"
  type        = bool
  default     = true
}

variable "allow_ssh" {
  description = "Allow external access to ssh port 22 on the Vault VMs. It is a best practice to set this to false,\nhowever it is true by default for the sake of backwards compatibility.\n"
  type        = bool
  default     = true
}

variable "http_proxy" {
  description = "HTTP proxy for downloading agents and vault executable on startup. Only necessary if allow_public_egress is false.\nThis is only used on the first startup of the Vault cluster and will NOT set the global HTTP_PROXY environment variable.\ni.e. If you configure Vault to manage credentials for other services, default HTTP routes will be taken.\n"
  type        = string
  default     = ""
}
...
variable "vault_tls_disable_client_certs" {
  description = "Use client certificates when provided. You may want to disable this if users will\nnot be authenticating to Vault with client certificates.\n"
  type        = string
  default     = false
}
...
variable "vault_tls_require_and_verify_client_cert" {
  description = "Always use client certificates. You may want to disable this if users will\nnot be authenticating to Vault with client certificates.\n"
  type        = string
  default     = false
}
